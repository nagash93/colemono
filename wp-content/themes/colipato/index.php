<?php get_header(); ?>
	...
	<div class="slider">
		...
		<?php while ( $slider_loop->have_posts() ) : $slider_loop->the_post(); $src = wp_get_attachment_image_src( get_post_thumbnail_id($post->ID), false, '' ); ?>
			<div class="featured-item" style="background:<?php echo get_post_meta($post->ID, 'feature_bg', true); ?> url(<?php echo $src[0]; ?>) no-repeat center; height:380px;">
	...
	<div id="main-wrapper">
		...
		<div id="main">
			...
			<div id="news-wrapper">
				...
				<div id="destacado">
					<h3 class="title-destacado">Noticias Destacadas</h3>
									<?php 
							
								 query_posts("cat=1246&showposts=3");
								if (have_posts()) : while (have_posts()) : the_post() ?>
								
							<!-- BEGIN POST -->
							<div id="post-destacado">
											
						<?php include( TEMPLATEPATH . '/includes/show-destacado.php' ); ?>
							</div>
							<!-- EMD POST -->
							<?php  endwhile; endif; ?>
				...
				<?php while (have_posts()) : the_post(); ?>
					...
					<?php elseif ( in_category('programas')): ?>
			
						<?php include( TEMPLATEPATH . '/includes/show-logpost.php' ); ?>
