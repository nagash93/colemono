...
function van_short_googlemap( $atts ) {
	extract(shortcode_atts(array('src'    => '','size'=>'','width' => '640','height' => '320'), $atts));

	if ( $size == "auto" ) {
		
		$output = '<div class="short-resp-container"><div class="short-resp">' . van_embed_code($src . '&amp;output=embed', $width, $height, 'overflow:hidden;', '') . '</div></div>';

	}else{

		$output = '<div style="width:' . $width . 'px;height:' . $height . 'px;margin:0 auto;">' . van_embed_code($src . '&amp;output=embed', $width, $height, 'overflow:hidden;', '') . '</div>';
	}
	return $output;
}
