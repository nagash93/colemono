...
function van_ajax_load_more() {
	...
	query_posts("post_status=publish&showposts=" . get_option('posts_per_page') . "&paged=" . $pageNum);
...
function van_post_view() {
	...
	$ips               = get_post_meta($post->ID, "van_viewers_ips");
	$viewer_ip     = isset( $ips[0] ) ? $ips[0] : "" ;
	...
	if (  is_singular() ) {
		...
		if (  in_array( $ip, array_keys($viewer_ip ) ) )  {
			...
			$time_now     = time(); 
			$viewer_time = $viewer_ip[$ip];  
			$diff  = abs($time_now - $viewer_time) ;

			if ( round( $diff / 60 )  > 120 ) 
				$return = true;
		}  
	}
	if( isset( $return ) && $return === true ){
		...
		update_post_meta($post->ID ,"van_viewers_ips", $viewer_ip);
	}
...
function van_excerpt_more( $more ) {
	return ' <a class="read-more" href="'. get_permalink( get_the_ID() ) . '">' . __( 'Leer más &rarr;', 'van' ) . '</a>';
}
